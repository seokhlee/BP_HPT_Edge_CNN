...
train_y = zeros([2 length(find(y1==1))+length(find(y1==2))]); % there are 10 labels in MNIST lables
...
% initialize cnn
cnn.namaste=10; % just intiationg cnn object
...
%% Example 4
cnn=cnnAddConvLayer(cnn, 18, [2 2], 'rect');
%cnn=cnnAddPoolLayer(cnn, 1, 'mean');
%cnn=cnnAddConvLayer(cnn, 1, [2 1], 'rect');
cnn=cnnAddPoolLayer(cnn, 3, 'mean');
cnn=cnnAddFCLayer(cnn,2, 'sigm' ); %add fully connected layer % last layer no of nodes = no of lables
