...
Datapath = 'D:\SeokhyeongLee\BP_project\BP_CTT_Paper\CNN\MNIST';
num1=0;num2=1;
...
train_y = zeros([2 length(find(y1==num1))+length(find(y1==num2))]); % there are 10 labels in MNIST lables
...
test_y = zeros([2 length(find(y2==num1))+length(find(y2==num2))]); % there are 10 labels in MNIST lables
count=0;
for i=1:num
    if y2(i)==num1
        count=count+1;
...
%cnn=cnnAddPoolLayer(cnn, 27, 'mean');
%cnn=cnnAddFCLayer(cnn,2, 'sigm' ); %add fully connected layer % last layer no of nodes = no of lables
